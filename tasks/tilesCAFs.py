...
from tasks.util import Time

TRANSLATED_HOR = False
TRANSLATED_VER = False
...
def score_tile(np_tile, tissue_percent, slide_num, row, col):
  ...
  score = tissue_percent + (random()) # choose randomly between 0 and 1 to add to the 100 tissue percent
...
Functions modified from previous work
...
def generate_tile_summaries(tile_sum, np_img, display=True, save_summary=False):
  ...
  original_img_path = slideCAFs.get_training_image_path(slide_num)
  np_orig = slideCAFs.open_image_np(original_img_path)
...
def save_tile_summary_image(pil_img, slide_num):
  ...
  """
  t = Time()
  filepath = slideCAFs.get_tile_summary_image_path(slide_num)
  pil_img.save(filepath)
  print("%-20s | Time: %-14s  Name: %s" % ("Save Tile Sum", str(t.elapsed()), filepath))

  t = Time()
  thumbnail_filepath = slideCAFs.get_tile_summary_thumbnail_path(slide_num)
  slideCAFs.save_thumbnail(pil_img, slideCAFs.THUMBNAIL_SIZE, thumbnail_filepath)
...
def save_top_tiles_image(pil_img, slide_num):
  ...
  """
  t = Time()
  filepath = slideCAFs.get_top_tiles_image_path(slide_num)
  pil_img.save(filepath)
  print("%-20s | Time: %-14s  Name: %s" % ("Save Top Tiles Image", str(t.elapsed()), filepath))

  t = Time()
  thumbnail_filepath = slideCAFs.get_top_tiles_thumbnail_path(slide_num)
  slideCAFs.save_thumbnail(pil_img, slideCAFs.THUMBNAIL_SIZE, thumbnail_filepath)
...
def save_tile_summary_on_original_image(pil_img, slide_num):
  ...
  """
  t = Time()
  filepath = slideCAFs.get_tile_summary_on_original_image_path(slide_num)
  pil_img.save(filepath)
  print("%-20s | Time: %-14s  Name: %s" % ("Save Tile Sum Orig", str(t.elapsed()), filepath))

  t = Time()
  thumbnail_filepath = slideCAFs.get_tile_summary_on_original_thumbnail_path(slide_num)
  slideCAFs.save_thumbnail(pil_img, slideCAFs.THUMBNAIL_SIZE, thumbnail_filepath)
  print(
...
def save_top_tiles_on_original_image(pil_img, slide_num):
  ...
  """
  t = Time()
  filepath = slideCAFs.get_top_tiles_on_original_image_path(slide_num)
  pil_img.save(filepath)
  print("%-20s | Time: %-14s  Name: %s" % ("Save Top Orig", str(t.elapsed()), filepath))

  t = Time()
  thumbnail_filepath = slideCAFs.get_top_tiles_on_original_thumbnail_path(slide_num)
  slideCAFs.save_thumbnail(pil_img, slideCAFs.THUMBNAIL_SIZE, thumbnail_filepath)
  print(
...
def summary_and_tiles(slide_num, display=True, save_summary=False, save_data=True, save_top_tiles=True):
  """
  Generate tile summary and top tiles for slideCAFs.
  ...
  """
  img_path = slideCAFs.get_filter_image_result(slide_num)
  np_img = slideCAFs.open_image_np(img_path)
...
def save_tile_data(tile_summary):
  ...
  data_path = slideCAFs.get_tile_data_path(tile_summary.slide_num)
...
def tile_to_pil_tile(tile):
  ...
  """
  t = tile
  slide_filepath = slideCAFs.get_training_slide_path(t.slide_num)
  s = slideCAFs.open_slide(slide_filepath)

  if TRANSLATED_HOR and not TRANSLATED_VER:
    x, y = (t.o_c_s + round(COL_TILE_SIZE / 2)), t.o_r_s
    print('if TRANSLATED_HOR and not TRANSLATED_VER:')

  elif TRANSLATED_VER and not TRANSLATED_HOR:
    x, y = t.o_c_s , (t.o_r_s + round(ROW_TILE_SIZE / 2))
    print('elif TRANSLATED_VER and not TRANSLATED_HOR:')
     
  elif TRANSLATED_HOR and TRANSLATED_VER:
    print("Cannot have the tiles translated both vertically and horizontally. Change parameters of tilesCAFs.py")
  else:
    x, y = t.o_c_s, t.o_r_s
    print("no condition")
...
def save_display_tile(tile, save=True, display=False):
  ...
  if save:
    t = Time()
    img_path = slideCAFs.get_tile_image_path(tile)
...
def score_tiles(slide_num, np_img=None, dimensions=None, small_tile_in_tile=False):
  ...
  if dimensions is None:
    img_path = slideCAFs.get_filter_image_result(slide_num)
    o_w, o_h, w, h = slideCAFs.parse_dimensions_from_image_filename(img_path)
  else:
  ...
  row_tile_size = round(ROW_TILE_SIZE / slideCAFs.SCALE_FACTOR)  # use round?
  col_tile_size = round(COL_TILE_SIZE / slideCAFs.SCALE_FACTOR)  # use round?
  ...
  tile_sum = TileSummary(slide_num=slide_num,
                         ...
                         tissue_percentage=filterCAFs.tissue_percent(np_img),
  ...
  for t in tile_indices:
    ...
    t_p = filterCAFs.tissue_percent(np_tile)
    ...
    if amount == TissueQuantity.HIGH:
      ...
      none += 1
    o_c_s, o_r_s = slideCAFs.small_to_large_mapping((c_s, r_s), (o_w, o_h))
    o_c_e, o_r_e = slideCAFs.small_to_large_mapping((c_e, r_e), (o_w, o_h))
...
def singleprocess_filtered_images_to_tiles(display=False, save_summary=True, save_data=True, save_top_tiles=True,
  ...
  if image_num_list is not None:
    ...
    print('loop 2')
    num_training_slides = slideCAFs.get_num_training_slides()
...
def multiprocess_filtered_images_to_tiles(display=False, save_summary=True, save_data=True, save_top_tiles=True,
  ...
  if save_summary and not os.path.exists(slideCAFs.TILE_SUMMARY_DIR):
...
def image_row(slide_num, tile_summary, data_link):
  ...
  """
  orig_img = slideCAFs.get_training_image_path(slide_num)
  orig_thumb = slideCAFs.get_training_thumbnail_path(slide_num)
  filt_img = slideCAFs.get_filter_image_result(slide_num)
  filt_thumb = slideCAFs.get_filter_thumbnail_result(slide_num)
  sum_img = slideCAFs.get_tile_summary_image_path(slide_num)
  sum_thumb = slideCAFs.get_tile_summary_thumbnail_path(slide_num)
  osum_img = slideCAFs.get_tile_summary_on_original_image_path(slide_num)
  osum_thumb = slideCAFs.get_tile_summary_on_original_thumbnail_path(slide_num)
  top_img = slideCAFs.get_top_tiles_image_path(slide_num)
  top_thumb = slideCAFs.get_top_tiles_thumbnail_path(slide_num)
  otop_img = slideCAFs.get_top_tiles_on_original_image_path(slide_num)
  otop_thumb = slideCAFs.get_top_tiles_on_original_thumbnail_path(slide_num)
  html = "    <tr>\n" + \
  ...
  if data_link:
    html += "        <div style=\"white-space: nowrap;\">S%03d Tile Summary\n" % slide_num + \
            "        (<a target=\"_blank\" href=\"%s\">Data</a>)</div>\n" % slideCAFs.get_tile_data_path(slide_num)
  else:
  ...
  for row in range(num_rows):
    ...
    for col in range(num_cols):
      ...
      if tile_num <= num_tiles:
        ...
        tile_img_path = slideCAFs.get_tile_image_path(t)
...
def generate_tiled_html_result(slide_nums, tile_summaries_dict, data_link):
  ...
  if not slideCAFs.TILE_SUMMARY_PAGINATE:
    html = ""
    html += filterCAFs.html_header("Tiles")
    ...
    html += filterCAFs.html_footer()
    text_file = open(os.path.join(slideCAFs.TILE_SUMMARY_HTML_DIR, "tiles.html"), "w")
    text_file.write(html)
    text_file.close()
  else:
    total_len = len(slide_nums)
    page_size = slideCAFs.TILE_SUMMARY_PAGINATION_SIZE
    ...
    for page_num in range(1, num_pages + 1):
      ...
      html = ""
      html += filterCAFs.html_header("Tiles, Page %d" % page_num)
      ...
      if page_num == 1:
        ...
        text_file = open(os.path.join(slideCAFs.TILE_SUMMARY_HTML_DIR, "tiles-%d.html" % page_num), "w")
      text_file.write(html)
      text_file.close()
...
def display_image_with_hsv_hue_histogram(np_rgb, text=None, scale_up=False):
  ...
  Args:
    ...
    scale_up: If True, scale up image to display by slideCAFs.SCALE_FACTOR
  """
  hsv = filterCAFs.filter_rgb_to_hsv(np_rgb)
  h = filterCAFs.filter_hsv_to_h(hsv)
  np_hist = np_hsv_hue_histogram(h)
  hist_r, hist_c, _ = np_hist.shape

  if scale_up:
    np_rgb = np.repeat(np_rgb, slideCAFs.SCALE_FACTOR, axis=1)
    np_rgb = np.repeat(np_rgb, slideCAFs.SCALE_FACTOR, axis=0)
...
def display_image_with_hsv_histograms(np_rgb, text=None, scale_up=False):
  ...
  """
  hsv = filterCAFs.filter_rgb_to_hsv(np_rgb)
  np_h = np_hsv_hue_histogram(filterCAFs.filter_hsv_to_h(hsv))
  np_s = np_hsv_saturation_histogram(filterCAFs.filter_hsv_to_s(hsv))
  np_v = np_hsv_value_histogram(filterCAFs.filter_hsv_to_v(hsv))
...
def rgb_to_hues(rgb):
  ...
  """
  hsv = filterCAFs.filter_rgb_to_hsv(rgb, display_np_info=False)
  h = filterCAFs.filter_hsv_to_h(hsv, display_np_info=False)
  return h
...
def hsv_saturation_and_value_factor(rgb):
  ...
  """
  hsv = filterCAFs.filter_rgb_to_hsv(rgb, display_np_info=False)
  s = filterCAFs.filter_hsv_to_s(hsv)
  v = filterCAFs.filter_hsv_to_v(hsv)
  s_std = np.std(s)
  v_std = np.std(v)
...
class TileSummary:
  ...
  orig_tile_w = None
  orig_tile_h = None
  scale_factor = slideCAFs.SCALE_FACTOR
  scaled_w = None
  scaled_h = None
  scaled_tile_w = None
  scaled_tile_h = None
...
def dynamic_tiles(slide_num, small_tile_in_tile=False):
  ...
  """
  np_img, large_w, large_h, small_w, small_h = slideCAFs.slide_to_scaled_np_image(slide_num)
  filt_np_img = filterCAFs.apply_image_filters(np_img)
